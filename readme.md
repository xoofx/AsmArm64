...
- **Easily disassemble** instructions and operands, including the knowledge of which operands and status flags are being read/write.
  ...
  - Inspect preferred aliases or their base instructions with `AsBaseInstruction()`, retaining the original encoding and correct operands.
  - Share `Arm64InstructionFormattingOptions` between single-instruction formatting and full disassembly: aliases, casing, numeric styles, and optional operands.
  - Resolve symbols and local labels, or explicitly choose relative-offset / absolute-address fallback for PC-relative targets.
